...
# Aside: A common form of f_i(t) is X_i*B
...
pphaz2 <- posterior_survfit(prior_exp_hazard2, 
                            type = 'haz', 
                            newdata = events,
                            prob = 0.5)
...
ggplot(pphaz2, aes(x = time, y = median,
  ...
  scale_y_continuous('hazard rate') +
...
# From this, we can estimate the survival
...
# This will be more relevant when covariates enter into the model, and when we 
#     are looking at a posterior fit.
# (Right now, all subjects have the same baseline hazard)
...
cowplot::plot_grid(rmst_check_plot(prior_exp_hazard) + xlim(0, 300), 
                   rmst_check_plot(prior_exp_hazard2) + xlim(0, 300), 
                   ncol = 1,
                   labels = c('default priors', 'updated to normal(0, 1)'))
...
rmst <- rmst_check(prior_ms_hazard_df_5_v2, tau = 300, newdata = events)
...
## Note: The frailty model currently hits a bug, reported here.
## To run these examples, you will need to install a different version of rstanarm
# 
## remotes::install_github('jburos/rstanarm', ref = 'stansurv/bugfix-survival', build_vignettes = FALSE)
#
